...
const start = async function(url) {
    let movies = movie1.split(', ');
    for (let i = 0; movies.length >= i; i++) {
        apiJSONResponse = await getJSON(`https://imdb-api.com/en/API/Search/${apiKey}/${movie1[i]}`);
        console.log(apiJSONResponse)
        let movieid = apiJSONResponse.results[0].id;
        let title = apiJSONResponse.results[0].title;
        // movieh1.innerHTML = title;
        console.log(apiJSONResponse)
        let movieratingres = await getJSON(`https://imdb-api.com/en/API/Ratings/${apiKey}/${movieid}`);
        let imdb = movie1ratingres.imDb * 10;
        let mc = movie1ratingres.imDb * 10;
        let score = Math.round((+movieratingres.rottenTomatoes + +imdb + + mc) / 3);
        scores.push(score)
    }
    console.log(scores)
    /*
